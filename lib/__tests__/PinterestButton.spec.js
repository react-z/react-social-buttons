...
test('FacebookLikeButton renders the correct elements and props', function () {
  ...
  var wrapper = (0, _enzyme.mount)(_react.default.createElement(_main.PinterestButton, {
    url: url
  }));
  expect(wrapper.instance().props.url).toEqual(url);
  expect(wrapper.find('#pinbutton').length).toEqual(1);
  expect(wrapper.find('img').length).toEqual(1);
  expect(wrapper.find('img').props().src).toContain('assets.pinterest.com'); // console.log(wrapper.debug())
});